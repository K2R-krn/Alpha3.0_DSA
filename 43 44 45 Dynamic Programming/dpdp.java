...
public class dpdp {
    ...
    public static int mcmTabu(int arr[]){
        ...
        // Initialization       // Also no need ot write this as default is 0 // But for logic we write it
        for(int i = 0;i<n;i++){
            dp[i][i] = 0;
        }
    ...
    public static int mcm(int arr[], int i, int j){
        ...
        int ans = Integer.MAX_VALUE;
        for(int k=i;k<=j-1;k++){
            int cost1 = mcm(arr, i, k); // Result size = Ai...Ak  = arr[i-1] x arr[k]
            int cost2 = mcm(arr, k+1, j);// Result Size = Ai+1...Aj  = arr[k] x arr[j]
